...
export const ExpandeHorarios = ({ expandedBarbeiroId, barbeiro, setShow, setShowModalDeletarHorario }) => {
  const { setBarbeiroSelecionado } = useContext(BarbeiroContext);
  const { setHorarioSelecionado, setHorarioOuBarbeiroPraExcluir } = useContext(HorarioContext);
          ...
          barbeiro.horarios.$values.map((horario, index) => (
            <li key={index} className="horario-item">
              ...
              <div className="botoes-bottom">
                <button className="btn-bottom editar"  onClick={() => handleUpdateHorario(horario)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="14"
                    height="14"
                ...
                <button className="btn-bottom excluir" onClick={() => handleDeleteHorario(horario)}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="14"
                    height="14"
